...
以下规则**不影响编译**（`var` 等写法可正常通过编译），仅作为编码期风格提示，由 `@arkmp/eslint-plugin` 以 `warn` 级报告。它们不对应编译期诊断码，也不写入「诊断码总表」——目的是在编码阶段就引导出更贴近 ArkTS 推荐写法的源码。

| 写法 | 建议 | 原因 |
| --- | --- | --- |
| `var` 声明 | 改用 `let` / `const` | ArkTS 建议避免 `var`（变量提升、函数作用域语义与块级作用域不一致）；ArkTS 子集与产物均为块级作用域，`var` 无收益 |
