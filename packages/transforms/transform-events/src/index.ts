...
export interface TransformEventsResult {
  ...
  /** 抽取的事件方法：方法名 → 回调体源码文本（未改写，见头部合并协议） */
...
export function transformEvents(model: ComponentModel): TransformEventsResult {
  ...
  const visitNode = (node: UINode): void => {
    ...
    for (const call of node.eventCalls) {
      ...
      methods[method] = call.body;
    }
    for (const child of node.children) visitChild(child);
  };

  const visitChild = (child: UIChildNode): void => {
    switch (child.type) {
      case 'component':
        visitNode(child);
        break;
      case 'if':
        for (const c of child.children) visitChild(c);
        for (const c of child.elseChildren) visitChild(c);
        break;
      case 'foreach':
        for (const c of child.children) visitChild(c);
        break;
